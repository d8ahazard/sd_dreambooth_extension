...
class ControlDataset(torch.utils.data.Dataset):
        ...
        for root, dirs, files in os.walk(self.instance_data_dir):
            for file in files:
                ...
                if is_image(file_path):
                    if "_control" in file_path:
                        continue
                    # Append _control to the image path, keeping the existing extension
                    control_file_path = file_path.rsplit('.', 1)[0] + '_control.' + file_path.rsplit('.', 1)[1]
                    if not os.path.isfile(control_file_path):
                        logger.warning(f"No control image found for {file_path}, skipping.")
                        continue
    ...
    def load_image(self, image_path, caption, res):
        control_image_path = image_path.rsplit('.', 1)[0] + '_control.' + image_path.rsplit('.', 1)[1]
        if self.debug_dataset:
            image = os.path.splitext(image_path)
            control_image = os.path.splitext(control_image_path)
            input_ids = caption
        else:
            if self.cache_latents:
                image = self.latents_cache[image_path]
                control_image = self.latents_cache[control_image_path]
            else:
                img = open_and_trim(image_path, res, False)
                image = self.image_transform(img)
                control_img = open_and_trim(control_image_path, res, False)
                control_image = self.image_transform(control_img)
            if self.shuffle_tags:
                caption, input_ids = self.cache_caption(image_path, caption)
            else:
                input_ids = self.caption_cache[image_path]
        return image, control_image, input_ids
    ...
    def make_buckets_with_caching(self):
        ...
        def cache_images(images, reso, p_bar: mytqdm):
            for image_path, cap in images:
                # Append _control to the image path, keeping the existing extension
                control_image_path = image_path.rsplit('.', 1)[0] + '_control.' + image_path.rsplit('.', 1)[1]
                try:
                    # If the image is not in the "precache",cache it
                    if image_path not in latents_cache:
                        if self.cache_latents and not self.debug_dataset:
                            self.cache_latent(image_path, reso)
                    # Otherwise, load it from existing cache
                    else:
                        self.latents_cache[image_path] = latents_cache[image_path]

                    # If the image is not in the "precache",cache it
                    if control_image_path not in latents_cache:
                        if self.cache_latents and not self.debug_dataset:
                            self.cache_latent(control_image_path, reso)
                    # Otherwise, load it from existing cache
                    else:
                        self.latents_cache[control_image_path] = latents_cache[control_image_path]
                    if not self.shuffle_tags:
                        self.cache_caption(image_path, cap)
                    self.sample_indices.append(image_path)
                    self.sample_cache.append((image_path, cap))
                    p_bar.update()
                except Exception as e:
                    traceback.print_exc()
                    print(f"Exception caching: {image_path}: {e}")
                    if image_path in self.caption_cache:
                        del self.caption_cache[image_path]
                    if (image_path, cap) in self.sample_cache:
                        del self.sample_cache[(image_path, cap)]
                    if image_path in self.sample_indices:
                        del self.sample_indices[image_path]
                    if image_path in self.latents_cache:
                        del self.latents_cache[image_path]
    ...
    def get_example(self, res):
        ...
        # Append _control to the image path, regardless of extension
        control_image_path = image_path.rsplit('.', 1)[0] + '_control.' + image_path.rsplit('.', 1)[1] 
    ...
    def __getitem__(self, index):
        ...
        control_image_path = image_path.rsplit('.', 1)[0] + '_control.' + image_path.rsplit('.', 1)[1]
        if not self.debug_dataset:
            image_data, control_image_data, input_ids = self.load_image(image_path, caption, self.active_resolution)
        else:
            image_data = image_path
            control_image_data = control_image_path
