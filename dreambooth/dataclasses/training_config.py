...
class TrainingConfig(BaseConfig):
    ...
    adam_beta1: float = Field(0.9, description="The beta1 parameter for the Adam optimizer.", title="Adam Beta 1", ge=0, le=1)
    ...
    checkpoints_total_limit: Optional[int] = Field(None,
                                                   description="[finetune] Max number of checkpoints to store.", title="Checkpoints Total Limit")
    clip_skip: int = Field(1, description="[db] Clip skip.", tile="Clip Skip")
    ...
    disable_logging: bool = Field(False, description="[db] Disable log parsing.", title="Disable Logging")
    epoch: int = Field(0, description="Lifetime trained epoch.", title="Epoch")
    epoch_pause_frequency: int = Field(0, description="[db] Epoch pause frequency.", title="Epoch Pause Frequency")
    epoch_pause_time: int = Field(0, description="[db] Epoch pause time.", title="Epoch Pause Time")
    ...
    gradient_accumulation_steps: int = Field(default=1,
                                             description="Number of updates steps to accumulate before performing a backward/update pass.", title="Gradient Accumulation Steps")
    ...
    graph_smoothing: float = Field(0.1, description="The scale of graph smoothing.", title="Graph Smoothing")
    hflip: bool = Field(False, description="Horizontal flip.", title="Horizontal Flip")
    input_pertubation: float = Field(0.1, description="[finetune] The scale of input pertubation. Recommended 0.1.", title="Input Pertubation")
    ...
    lora_model_name: str = Field("", description="[db] LoRA model name.", title="LoRA Model Name")
    lora_txt_rank: int = Field(4, description="[db] LoRA text rank.", title="LoRA Text Rank")
    lora_txt_weight: float = Field(1.0, description="[db] LoRA text weight.", title="LoRA Text Weight")
    lora_unet_rank: int = Field(4, description="[db] LoRA UNet rank.", title="LoRA UNet Rank")
    lora_weight: float = Field(1.0, description="[db] LoRA weight.", title="LoRA Weight")
    lr_factor: float = Field(0.5, description="Learning rate factor.", title="Learning rate factor")  # ato
    lr_num_cycles: int = Field(1, description="Learning rate cycles.", title="Learning rate cycle")  # ato
    lr_power: float = Field(1.0, description="Learning rate power.", title="Learning rate power")  # ato
    ...
    lr_warmup_steps: int = Field(500, description="Number of steps for the warmup in the lr scheduler.", title="Learning Rate Warmup Steps")
    max_grad_norm: float = Field(1.0, description="[finetune] Max gradient norm.", title="Max Gradient Norm")
    max_token_length: int = Field(75, description="[db] Max token length.", title="Max Token Length")
    max_train_samples: Optional[int] = Field(default=None, description="[finetune, controlnet] For debugging purposes or quicker training, truncate the number of training examples to this value if set.", title="Max Train Samples")
    ...
    model_dir: str = Field("", description="Model directory.", title="Model Directory")
    model_name: str = Field("", description="Model name.", title="Model Name")
    model_path: str = Field("", description="Model path.", title="Model Path")
    noise_scheduler: str = Field("DDPM", description="[db] Noise scheduler during training?.", title="Noise Scheduler")
    num_save_samples: int = Field(4, description="[finetune, controlnet] Number of samples to save.", title="Number of Save Samples")
    num_train_epochs: int = Field(100, description="Number of training epochs.", title="Number of Training Epochs")
    offset_noise: float = Field(0, description="[finetune, db] The scale of noise offset.", title="Offset Noise")
    ...
    pretrained_model_name_or_path: str = Field("", description="Pretrained model name or path.", title="Pretrained Model Name or Path")
    ...
    prior_loss_target: int = Field(100, description="[db] Prior loss target.", title="Prior Loss Target")
    prior_loss_weight: float = Field(0.75, description="[db] Prior loss weight.", title="Prior Loss Weight")
    prior_loss_weight_min: float = Field(0.1, description="[db] Minimum prior loss weight.", title="Minimum Prior Loss Weight")
    proportion_empty_prompts: float = Field(default=0,
                                            description="[controlnet] Proportion of image prompts to be replaced with empty strings. Defaults to 0 (no prompt replacement).", title="Proportion Empty Prompts")
    random_flip: bool = Field(False,
                              description="[finetune] Whether to randomly flip images horizontally.", title="Random Flip")
    resolution: int = Field(512, description="Maximum resolution for input images.", title="Resolution", ge=8,multiple_of=8)
    revision: int = Field(0, description="Model Revision.", title="Revision")
    sample_batch_size: int = Field(1, description="[db] Sample batch size.", title="Sample Batch Size", ge=1, le=100)
    ...
    scheduler: str = Field("ddim", description="Scheduler.", title="Scheduler")
    seed: int = Field(420420, description="Seed for reproducability, sanity prompt.", title="Seed", ge=-1, le=21474836147)
    shuffle_tags: bool = Field(True, description="[db] Shuffle tags.", title="Shuffle Tags")
    ...
    snr_gamma: Optional[float] = Field(5.0,
                                       description="[finetune, controlnet] SNR weighting gamma to be used if rebalancing the loss. Recommended value is 5.0.", title="SNR Gamma")
    split_loss: bool = Field(True, description="Split loss.", title="Split Loss")
    src: str = Field("", description="The source checkpoint.", title="Source Checkpoint")
    stop_text_encoder: float = Field(1.0, description="[db, controlnet] Stop text encoder.", title="Stop Text Encoder Percentage")
    ...
    tenc_weight_decay: float = Field(0.01, description="[db] Text encoder weight decay.", title="Text Encoder Weight Decay")
    tenc_grad_clip_norm: float = Field(0.00, description="[db] Text encoder gradient clipping norm.", title="Text Encoder Gradient Clipping Norm")
    tomesd: float = Field(0, description="[db] TomesD.", title="TomesD")
    train_batch_size: int = Field(1, description="Batch size for the training dataloader.", title="Train Batch Size", gt=0)
    ...
    txt_learning_rate: float = Field(5e-6, description="[db] Text learning rate.", title="Text Learning Rate")
    ...
    train_lora: bool = Field(False, description="[db] Use LoRA.", title="Use LoRA")
    use_lora_extended: bool = Field(False, description="[db] Use LoRA extended.", title="Use LoRA Extended")
    use_subdir: bool = Field(False, description="[db] Use subdirectory when saving weights.", title="Use Subdirectory")
    v2: bool = Field(False, description="If this is a V2 Model or not.", title="V2")
