...
def render():
    ...
    with gr.Accordion(open=False, label="Performance") as PERFORMANCE_ACCORDION:
        ...
        TIMESTEP_BIAS_BEGIN = gr.Slider(label='Timestep Bias Begin', value=0, visible=True, step=1, minimum=0, maximum=100000)
        TIMESTEP_BIAS_END = gr.Slider(label='Timestep Bias End', value=1000, visible=True, step=1, minimum=0, maximum=100000)
        TIMESTEP_BIAS_MULTIPLIER = gr.Slider(label='Timestep Bias Multiplier', value=1.0, visible=True, step=0.01, minimum=0, maximum=1)
        TIMESTEP_BIAS_PORTION = gr.Slider(label='Timestep Bias Portion', value=0.25, visible=True, step=0.01, minimum=0, maximum=1)
        TIMESTEP_BIAS_STRATEGY = gr.Dropdown(label='Timestep Bias Strategy', choices=['earlier', 'later', 'range', 'none'], value='none', visible=True)
        USE_8BIT_ADAM = gr.Checkbox(label='Use 8Bit Adam', value=True, visible=True)
        USE_EMA = gr.Checkbox(label='Use Ema', value=False, visible=True)
