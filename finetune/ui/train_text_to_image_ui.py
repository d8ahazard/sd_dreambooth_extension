...
def render():
    ...
    with gr.Accordion(open=False, label="Dataset") as DATASET_ACCORDION:
        ...
        INPUT_PERTURBATION = gr.Slider(label='Input Perturbation', value=0.1, visible=True, step=0.1, minimum=0, maximum=1)
    ...
    with gr.Accordion(open=False, label="Validation") as VALIDATION_ACCORDION:
        VALIDATION_PROMPTS = gr.Textbox(label='Validation Prompts', value='None', visible=True)
