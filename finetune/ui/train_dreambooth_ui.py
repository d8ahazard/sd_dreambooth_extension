...
def render():
    ...
    with gr.Accordion(open=False, label="Performance") as PERFORMANCE_ACCORDION:
        ...
        OFFSET_NOISE = gr.Checkbox(label='Offset Noise', value=False, visible=True)
    ...
    with gr.Accordion(open=False, label="Saving") as SAVING_ACCORDION:
        ...
        SKIP_SAVE_TEXT_ENCODER = gr.Checkbox(label='Skip Save Text Encoder', value=False, visible=True)
    with gr.Accordion(open=False, label="Validation") as VALIDATION_ACCORDION:
        ...
        VALIDATION_SCHEDULER = gr.Dropdown(label='Validation Scheduler', choices=['DPMSolverMultistepScheduler', 'DDPMScheduler'], value='DPMSolverMultistepScheduler', visible=True)
